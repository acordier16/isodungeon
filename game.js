...
export class Game {
    ...
    applyActionToEntity(action, entity) {
        for (var i = 0; i < action.effects.length; i++) {
            ...
            var clonedEffect = new Effect(effect.type, effect.duration, effect.allowMultiple, effect.probability, effect.deltaPV, effect.deltaPM, effect.deltaPA, effect.deltaPO);
    ...
    run() {
        ...
        $("#isocanvas").on('mousemove', function(e) {
            ...
            self.isometric.selectedTileX = Math.round(e.pageX / self.isometric.tileColumnOffset - e.pageY / self.isometric.tileRowOffset);
            self.isometric.selectedTileY = Math.round(e.pageX / self.isometric.tileColumnOffset + e.pageY / self.isometric.tileRowOffset);
            ...
            if (self.isometric.isCursorOnMap()) {
                ...
                var entity = self.isometric.entityAtThisPosition(self.isometric.selectedTileX, self.isometric.selectedTileY, self.entities)
                if (self.phase == "PLAYER_TURN_MOVE") {
                    ...
                    // else if no entity, check if within player's PM range (if player's turn, in not in action mode)
                        ...
                        var path = self.map.pathFromEntityToTile(self.isometric.selectedTileX, self.isometric.selectedTileY, self.entities[0]);
                        if (path.length > 0 && path.length <= self.entities[0].PM) {
                            self.isometric.drawDiamond(self.entities[0].x, self.entities[0].y, 'orange', true, 'orange', 0.5);
        ...
        $("#isocanvas").on('click', function(e) {
            ...
            if (self.phase == "PLAYER_TURN_MOVE") {
                ...
                if (self.isometric.isCursorOnMap()) {
                    if (self.isometric.entityAtThisPosition(self.isometric.selectedTileX, self.isometric.selectedTileY, self.entities) == null) {
                        var path = self.map.pathFromEntityToTile(self.isometric.selectedTileX, self.isometric.selectedTileY, self.entities[0]);
                        if (path.length > 0 && path.length <= self.entities[0].PM) {
            ...
            if (self.phase == "PLAYER_TURN_ACTION") {
                if (self.isometric.isCursorOnMap()) {
                    ...
                    if (self.isometric.tileWithinActionRange(self.isometric.selectedTileX, self.isometric.selectedTileY, self.entities[0], self.chosenAction)) {
                        var entity = self.isometric.entityAtThisPosition(self.isometric.selectedTileX, self.isometric.selectedTileY, self.entities);
